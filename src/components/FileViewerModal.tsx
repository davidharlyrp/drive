...
import { X, ChevronLeft, ChevronRight, File as FileIcon, Trash2, Edit2, FolderInput } from 'lucide-react';
...
export function FileViewerModal({ file, files, onClose, onRefetch, onRename, onMove, onDelete, onDownload }: FileViewerModalProps) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-white/40 backdrop-blur-2xl animate-in fade-in duration-500" onClick={onClose}>
            ...
            <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-8 py-2 bg-white/40 border-b border-surface-100 z-10 backdrop-blur-md" onClick={e => e.stopPropagation()}>
                <div className="flex flex-col">
                    <span className="text-xl font-semibold text-surface-900 truncate">{activeFile.name}</span>
                    <p className="text-[10px] font-semibold text-surface-400 mt-0.5">
                        {currentIndex + 1} of {files.length || 1} items
                    </p>
                </div>
            ...
            {/* Content Viewer */}
            <div className="relative w-full h-full flex items-center justify-center p-8 sm:p-24" onClick={e => e.stopPropagation()}>
                {activeIsImage && (
                    <div className="relative group p-4 bg-white rounded-[2rem] shadow-premium">
                        <img
                            src={fileUrl}
                            alt={activeFile.name}
                            className="max-w-full max-h-[75vh] object-contain select-none rounded-[1.5rem]"
                        />
                    </div>
                )}

                {activeIsVideo && (
                    <div className="relative p-2 bg-white rounded-[2rem] shadow-premium overflow-hidden">
                        <video
                            key={activeFile.id}
                            src={fileUrl}
                            controls
                            ...
                            muted
                            playsInline
                            className="max-w-full max-h-[75vh] rounded-[1.8rem] shadow-2xl"
                        />
                    </div>
                )}
                ...
                {isNavigatable && files.length > 1 && (
                    <>
                        <button
                            className="absolute left-6 sm:left-10 p-4 sm:p-5 rounded-xl bg-white/80 text-surface-900 border border-surface-100 shadow-premium hover:bg-brand hover:text-white transition-all z-20 active:scale-90"
                            onClick={handlePrev}
                        >
                            <ChevronLeft size={24} />
                        </button>
                        <button
                            className="absolute right-6 sm:right-10 p-4 sm:p-5 rounded-xl bg-white/80 text-surface-900 border border-surface-100 shadow-premium hover:bg-brand hover:text-white transition-all z-20 active:scale-90"
                            onClick={handleNext}
                        >
